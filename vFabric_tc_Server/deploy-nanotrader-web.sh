...
export context=spring-nanotrader-web


echo "WAR File = $war_file"
#export war_file=https://artifactory.slatchdev.local/artifactory/binrepo/springtrader/spring-nanotrader-web-1.0.1.war
wget --no-check-certificate $war_file
war_ext=$(echo $war_file | awk -F'/' ' { print $(NF) } ')
mv $war_ext /tmp
export war_file="/tmp/$war_ext"
